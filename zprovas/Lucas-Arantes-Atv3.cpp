...
int main()
{
    ...
    entrada_cliente(&fila_caixa, "Joao Felipe Caldeira", 35, 2, 141.39);
    ...
    entrada_cliente(&fila_caixa, "Ana Julia da Cunha", 72, 15, 186.69);
    ...
    cout << "\t Busca por Gastos Minimos " << endl;
    cout << "\t ========================" << endl;
...
void dados_primeiro_fila(Fila *fila)
{
    if (fila->inicio != NULL)
    {
        cout << "Proximo sera: \t \t" << fila->inicio->cliente.senha << "  -  ";
...
void dados_ultimo_fila(Fila *fila)
{
    if (fila->inicio != NULL)
    {
        cout << "Ultimo da fila:  \t" << fila->final->cliente.senha << "  -  ";
...
void buscar_clientes_senha(Fila *fila, int senha)
{
    ...
    while (aux->next != NULL)
    {
        if (aux->cliente.senha == senha)
        {
            cout << "[" << aux->cliente.senha << "-";
            cout << aux->cliente.nome << "]\n";
            break;
        }
        aux = aux->next;
    }
...
void buscar_clientes_menores_idade(Fila *fila)
{
    ...
    while (aux->next != NULL)
    {
        if (aux->cliente.idade < 18)
        {
            cout << "[" << aux->cliente.nome << "-";
            cout << aux->cliente.idade << " anos]";
        }
        aux = aux->next;
    }
    cout << '\n';
...
void buscar_clientes_melhor_idade(Fila *fila)
{
    ...
    for (Node *aux = fila->inicio; aux->next != NULL; aux = aux->next)
    {
        if (aux->cliente.idade > 65)
        {
            cout << "[" << aux->cliente.nome << "-";
            cout << aux->cliente.idade << " anos]";
        }
...
void buscar_clientes_valores_quantidades(Fila *fila, int quant_itens)
{
    if (fila->inicio == NULL)
    {
        cout << "nao tem ninguem na fila\n";
    ...
    for (Node *aux = fila->inicio; aux->next != NULL; aux = aux->next)
    {
        if (aux->cliente.quantidade_itens > quant_itens)
        {
            cout << "[" << aux->cliente.nome << "-";
            cout << "quant = " << aux->cliente.quantidade_itens << "]";
        }
    }
    cout << '\n';
}

void buscar_clientes_valores_compras(Fila *fila, float valor)
{
    if (fila->inicio == NULL)
    {
        cout << "nao tem ninguem na fila\n";
    ...
    for (Node *aux = fila->inicio; aux->next != NULL; aux = aux->next)
    {
        if (aux->cliente.valor > valor)
        {
            cout << "[" << aux->cliente.nome << "-";
            cout << "R$ " << aux->cliente.valor << "]";
        }
    }
}
