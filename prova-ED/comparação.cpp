...
//  cout << "Posi̤̣o: " << posicao <<endl;
    if ( lista->tamanho < posicao){
        cout << endl << "Tamanho da lista menor que a posi̤̣o informada" << endl;
    } else {
...
/* Fun̵̤es devem ser desenvolvidas a partir daqui */
    ...
    if ( lista->tamanho < posicao){
        ...
        if (posicao == 1) {
            ...
            while (posicao >= 0) {
                posicao--;
                if (posicao == 0) {
                    cout << "Removido: " << aux->tarefa.tarefa << endl;
                    aux->proximo->anterior = aux->anterior;
                    aux->anterior->proximo = aux->proximo;
                    aux->proximo = NULL;
                    aux->anterior = NULL;
...
void buscar_tarefas_por_categoria(Lista *lista, int categoria) {
    if (lista->inicio != NULL) {
        ...
        Node* cliente_local = lista->inicio;
        while(cliente_local != NULL){
            if (cliente_local->tarefa.categoria == categoria) {
                cout <<"[" << cliente_local->tarefa.tarefa << "]";
            }
            cliente_local = cliente_local->proximo;
        }
...
void buscar_tarefas_por_prioridades(Lista *lista, int prioridade){
    if (lista->inicio != NULL) {
        ...
        Node* cliente_local = lista->inicio;
        while(cliente_local != NULL){
            if (cliente_local->tarefa.prioridade == prioridade) {
                cout <<"[" << cliente_local->tarefa.tarefa << "]";
            }
            cliente_local = cliente_local->proximo;
        }
